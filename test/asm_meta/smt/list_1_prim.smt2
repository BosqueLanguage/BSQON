...
(define-fun Main@main ((f List<Int>)) (@Result List<Int>)
    (ite (not (= (let ((@tmp-2-1 (let ((@tmp-1-0 (ite (not (List<Int>@get_$_precond0 f 0)) ((as @Result-err (@Result Int)) @err-other) (@Result-ok (List<Int>@get f 0))))) (ite (not (is-@Result-ok @tmp-1-0)) ((as @Result-err (@Result Bool)) (@Result-etag @tmp-1-0)) (@Result-ok (not (= (@Result-value @tmp-1-0) 27))))))) (ite (not (is-@Result-ok @tmp-2-1)) @tmp-2-1 (@Result-ok (or (not (= (List<Int>@size f) 1)) (@Result-value @tmp-2-1))))) (@Result-ok true))) (let ((@failure (let ((@tmp-2-1 (let ((@tmp-1-0 (ite (not (List<Int>@get_$_precond0 f 0)) ((as @Result-err (@Result Int)) @err-other) (@Result-ok (List<Int>@get f 0))))) (ite (not (is-@Result-ok @tmp-1-0)) ((as @Result-err (@Result Bool)) (@Result-etag @tmp-1-0)) (@Result-ok (not (= (@Result-value @tmp-1-0) 27))))))) (ite (not (is-@Result-ok @tmp-2-1)) @tmp-2-1 (@Result-ok (or (not (= (List<Int>@size f) 1)) (@Result-value @tmp-2-1))))))) (ite (= @failure (@Result-ok false)) ((as @Result-err (@Result List<Int>)) @err-other) ((as @Result-err (@Result List<Int>)) (@Result-etag @failure))))
        (@Result-ok f)
    )
)
