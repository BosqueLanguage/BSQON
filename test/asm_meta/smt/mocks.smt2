...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(declare-const forecast Main@Forecast)
(declare-const rentalinfo Main@RentalAvailabilityInfo)
(declare-const requestedQuantity Nat)
(declare-const allowPartials Bool)

(declare-const res (@Result (@EList-2 Nat Main@Reason)))

(assert (= res (Main@main forecast rentalinfo requestedQuantity allowPartials)))
(assert (= res ((as @Result-err (@Result (@EList-2 Nat Main@Reason))) @err-other)))
