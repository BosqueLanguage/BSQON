...
export function userEmailNotEmpty(x) {
    if(x._$isNone()) {
        let $x = x._$asNone("expected None but got Some @ pre.bsq:33"); {
            return false;
        }
    }
...
export function userNameNotEmpty(x) {
    if(x._$isNone()) {
        let $x = x._$asNone("expected None but got Some @ pre.bsq:40"); {
            return false;
        }
    }
...
export function userPhoneNotEmpty(x) {
    if(x._$isNone()) {
        let $x = x._$asNone("expected None but got Some @ pre.bsq:47"); {
            return false;
        }
    }
...
export function userAddressNotEmpty(x) {
    if(x._$isNone()) {
        let $x = x._$asNone("expected None but got Some @ pre.bsq:54"); {
            return false;
        }
    }
...
export const ALPHANUMERIC = Object.create($VRepr, {
    ...
    $create: { value: (value) => {
        _$formatchk(_$accepts('/ [a-zA-Z0-9]+/c', value, "Main"), "failed regex @ pre.bsq:4");
...
export const EMAIL = Object.create($VRepr, {
    ...
    $create: { value: (value) => {
        _$formatchk(_$accepts('/${Main::re_SAFEIDENTIFIERCHAR}+(\'.\'${Main::re_SAFEIDENTIFIERCHAR})* \'@\' (${Main::re_SAFEIDENTIFIERCHAR}+ \'.\')+ [a-zA-Z]+ /c', value, "Main"), "failed regex @ pre.bsq:5");
...
export const USER_PHONE = Object.create($VRepr, {
    ...
    $create: { value: (value) => {
        _$formatchk(_$accepts('/[+]? [1-9][0-9]?\' \'* \'(\'? [0-9]{3} \')\'? [ -]?[0-9]{3}[ -]?[0-9]{4} /c', value, "Main"), "failed regex @ pre.bsq:7");
...
export const USER_NAME = Object.create($VRepr, {
    ...
    $create: { value: (value) => {
        _$formatchk(_$accepts('/ ${Main::re_USERNAME} /c', value, "Main"), "failed regex @ pre.bsq:12");
...
export const USER_ADDRESS = Object.create($VRepr, {
    ...
    $create: { value: (value) => {
        _$formatchk(_$accepts('/[0-9a-zA-Z -,.#()]+/c', value, "Main"), "failed regex @ pre.bsq:14");
...
export const UserDTO = Object.create($VRepr, {
    ...
    $checkinv_19_660: { value: ($email, $password, $name, $phone, $address, $id) => userEmailNotEmpty($email) },
    $checkinv_21_735: { value: ($email, $password, $name, $phone, $address, $id) => ($Core.CString.size.call($password.value) >= 6n) && ($Core.CString.size.call($password.value) <= 50n) },
    $checkinv_23_847: { value: ($email, $password, $name, $phone, $address, $id) => userNameNotEmpty($name) },
    $checkinv_25_923: { value: ($email, $password, $name, $phone, $address, $id) => userPhoneNotEmpty($phone) },
    $checkinv_27_1005: { value: ($email, $password, $name, $phone, $address, $id) => userAddressNotEmpty($address) },
    $checkinv_29_1063: { value: ($email, $password, $name, $phone, $address, $id) => $id > 0n },
    $create: { value: (email, password, name, phone, address, id) => {
        _$invariant(UserDTO.$checkinv_19_660(email, password, name, phone, address, id), "failed invariant @ pre.bsq:19");
        _$invariant(UserDTO.$checkinv_21_735(email, password, name, phone, address, id), "failed invariant @ pre.bsq:21");
        _$invariant(UserDTO.$checkinv_23_847(email, password, name, phone, address, id), "failed invariant @ pre.bsq:23");
        _$invariant(UserDTO.$checkinv_25_923(email, password, name, phone, address, id), "failed invariant @ pre.bsq:25");
        _$invariant(UserDTO.$checkinv_27_1005(email, password, name, phone, address, id), "failed invariant @ pre.bsq:27");
        _$invariant(UserDTO.$checkinv_29_1063(email, password, name, phone, address, id), "failed invariant @ pre.bsq:29");
        return Object.create(UserDTO, { email: { value: email, enumerable: true }, password: { value: password, enumerable: true }, name: { value: name, enumerable: true }, phone: { value: phone, enumerable: true }, address: { value: address, enumerable: true }, id: { value: id, enumerable: true } });
    } },
    $update: { value: function(updates) {
        let vobj = {...this, ...updates};
        _$invariant(UserDTO.$checkinv_19_660(vobj.email, vobj.password, vobj.name, vobj.phone, vobj.address, vobj.id), "failed invariant @ pre.bsq:19");
        _$invariant(UserDTO.$checkinv_21_735(vobj.email, vobj.password, vobj.name, vobj.phone, vobj.address, vobj.id), "failed invariant @ pre.bsq:21");
        _$invariant(UserDTO.$checkinv_23_847(vobj.email, vobj.password, vobj.name, vobj.phone, vobj.address, vobj.id), "failed invariant @ pre.bsq:23");
        _$invariant(UserDTO.$checkinv_25_923(vobj.email, vobj.password, vobj.name, vobj.phone, vobj.address, vobj.id), "failed invariant @ pre.bsq:25");
        _$invariant(UserDTO.$checkinv_27_1005(vobj.email, vobj.password, vobj.name, vobj.phone, vobj.address, vobj.id), "failed invariant @ pre.bsq:27");
        _$invariant(UserDTO.$checkinv_29_1063(vobj.email, vobj.password, vobj.name, vobj.phone, vobj.address, vobj.id), "failed invariant @ pre.bsq:29");
        return Object.assign(Object.create(UserDTO), vobj);
    } },
    $createAPI: { value: (email, password, name, phone, address, id) => {
        _$invariant(UserDTO.$checkinv_19_660(email, password, name, phone, address, id), "failed invariant @ pre.bsq:19");
        _$invariant(UserDTO.$checkinv_21_735(email, password, name, phone, address, id), "failed invariant @ pre.bsq:21");
        _$invariant(UserDTO.$checkinv_23_847(email, password, name, phone, address, id), "failed invariant @ pre.bsq:23");
        _$invariant(UserDTO.$checkinv_25_923(email, password, name, phone, address, id), "failed invariant @ pre.bsq:25");
    ...
    $parseAPI: { value: (parser) => { parser.checkConsType("Main::UserDTO"); const vals = parser.parseArgListGeneral([["email", "Option<Main::EMAIL>"],["password", "Main::ALPHANUMERIC"],["name", "Option<Main::USER_NAME>"],["phone", "Option<Main::USER_PHONE>"],["address", "Option<Main::USER_ADDRESS>"],["id", "Int"]]); return UserDTO.$createAPI(...vals); } },
    $emitAPI: { value: (emitter, value) => { return "Main::UserDTO{" + emitter.emitValue("Option<Main::EMAIL>", value.email) + ", " + emitter.emitValue("Main::ALPHANUMERIC", value.password) + ", " + emitter.emitValue("Option<Main::USER_NAME>", value.name) + ", " + emitter.emitValue("Option<Main::USER_PHONE>", value.phone) + ", " + emitter.emitValue("Option<Main::USER_ADDRESS>", value.address) + ", " + emitter.emitValue("Int", value.id) + "}"; } },
    $scall: { value: function(name, tt, ...args) { return this[name][tt].call(this, ...args); } }
});
...
let res; try { res = main(); } catch(e) { process.stdout.write(`Error -- ${e.$info || e}\n`); process.exit(1); }
