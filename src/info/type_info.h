...
{
//
// TODO: this is not very performant (easy to debug though) we should do a numeric ID map or intern to string pointers
// later
//
...
class TypeAnnotationInfo
{
  public:
    ...
    TypeAnnotationInfo(std::string docstring, std::optional<std::vector<std::pair<TypeKey, std::string>>> sensitivetags)
...
class TypedeclType : public EntityType
{
  public:
    ...
    TypedeclType(TypeKey tkey, std::vector<TypeKey> supertypes, TypeAnnotationInfo annotations, TypeKey primitivetype,
                 std::optional<std::pair<std::u8string, std::string>> optOfValidator, bool hasvalidations)
...
class APIFailedType : public EntityType
{
  public:
    ...
    APIFailedType(TypeKey ttype, std::vector<TypeKey> supertypes, TypeAnnotationInfo annotations)
        : EntityType(TypeTag::TYPE_APIFAILED, "APIResult<" + ttype + ">::Failed", supertypes, annotations), ttype(ttype)
    {
        ;
    }
...
class APIErrorType : public EntityType
{
  public:
    ...
    APIErrorType(TypeKey ttype, std::vector<TypeKey> supertypes, TypeAnnotationInfo annotations)
        : EntityType(TypeTag::TYPE_APIERROR, "APIResult<" + ttype + ">::Error", supertypes, annotations), ttype(ttype)
    {
        ;
    }
