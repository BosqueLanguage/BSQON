...
ValueSetPartition ValueSetGenerator::generateSome(const bsqon::SomeType *t, const ValueSetGeneratorEnvironment &env)
{
    ...
    auto tenv = env.step(pathAccessField(env.path, "value"), env.constraints, env.context);
    return this->generateType(oftype, tenv);
}
...
bool TestGenerator::isConstrainedLengthValue(const VCPath &currpath, bsqon::Value *&value)
{
    ...
    auto lc = std::find_if(this->vspartition->components.cbegin(), this->vspartition->components.cend(), [&currpath](const ValueComponent *vc)
                           { return vc->path == currpath; });
...
{
    auto ci = std::find_if(this->vspartition->components.cbegin(), this->vspartition->components.cend(), [&currpath](const ValueComponent *vc)
                           { return vc->path == currpath; });
...
bool TestGenerator::checkConstraintSatisfiability(const std::vector<const ValueConstraint *> &constraints)
{
    ...
    std::copy_if(constraints.cbegin(), constraints.cend(), std::back_inserter(oftypeconstraints), [](const ValueConstraint *vc)
                 { return dynamic_cast<const OfTypeConstraint *>(vc) != nullptr; });
    ...
    std::copy_if(constraints.cbegin(), constraints.cend(), std::back_inserter(minlenconstraints), [](const ValueConstraint *vc)
                 { return dynamic_cast<const MinLengthConstraint *>(vc) != nullptr; });
