...
#define BREX_ASSERT(condition, msg)                                            \
  if (!(condition)) {                                                          \
    brex::processAssert(__FILE__, __LINE__, msg);                              \
  }
#else
...
public:
  ...
  UnicodeRegexIterator(UnicodeString *sstr)
      : sstr(sstr), spos(0), epos(sstr->size() - 1), curr(0) {
    ;
  }
  UnicodeRegexIterator(UnicodeString *sstr, int64_t spos, int64_t epos,
                       int64_t curr)
      : sstr(sstr), spos(spos), epos(epos), curr(curr) {
    ;
  }
  ...
  inline void inc() {
    // if this is a multibyte char then advance by the number of bytes -- fast
    // path on single byte
  ...
  inline RegexChar get() const {
    // if this is a multibyte char then decode the number of bytes -- fast path
    // on single byte
...
public:
  ...
  CRegexIterator(const CString *sstr)
      : sstr(sstr), spos(0), epos(sstr->size() - 1), curr(0) {
    ;
  }
  CRegexIterator(const CString *sstr, int64_t spos, int64_t epos, int64_t curr)
      : sstr(sstr), spos(spos), epos(epos), curr(curr) {
    ;
  }
  ...
  inline void inc() { this->curr++; }

  inline void dec() { this->curr--; }

  inline RegexChar get() const { return (RegexChar)this->sstr->at(this->curr); }
};
...
std::optional<RegexChar> decodeHexEscapeAsRegex(const uint8_t *s,
                                                const uint8_t *e);
std::optional<UnicodeString> decodeHexEscapeAsUnicode(const uint8_t *s,
                                                      const uint8_t *e);
std::optional<CString> decodeHexEscapeAsC(const uint8_t *s, const uint8_t *e);
std::vector<uint8_t>
extractRegexCharToBytes(RegexChar rc); // utf8 encoded but no escaping

// Take a bytebuffer (of utf8 bytes) with escapes and convert to/from a
// UnicodeString
std::pair<std::optional<UnicodeString>, std::optional<std::u8string>>
unescapeUnicodeString(const uint8_t *bytes, size_t length);
std::pair<std::optional<UnicodeString>, std::optional<std::u8string>>
unescapeUnicodeStringLiteralInclMultiline(const uint8_t *bytes, size_t length);
...
std::pair<std::optional<CString>, std::optional<std::u8string>>
unescapeCString(const uint8_t *bytes, size_t length);
std::pair<std::optional<CString>, std::optional<std::u8string>>
unescapeCStringLiteralInclMultiline(const uint8_t *bytes, size_t length);
std::vector<uint8_t> escapeCString(const CString &sv);

// Take a bytebuffer regex literal (of utf8 bytes or chat bytes) with escapes
// and convert to/from a vector of RegexChars
std::optional<std::vector<RegexChar>>
unescapeUnicodeRegexLiteral(const uint8_t *bytes, size_t length);
std::optional<std::vector<RegexChar>>
unescapeCRegexLiteral(const uint8_t *bytes, size_t length);

// Take a bytebuffer regex char range element (of utf8 bytes or char bytes) with
// escapes and convert to a RegexChar
std::optional<RegexChar> unescapeSingleUnicodeRegexChar(const uint8_t *s,
                                                        const uint8_t *e);
std::optional<RegexChar> unescapeSingleCRegexChar(const uint8_t *s,
                                                  const uint8_t *e);

std::vector<uint8_t> escapeSingleUnicodeRegexChar(RegexChar c);
std::vector<uint8_t>
escapeRegexUnicodeLiteralCharBuffer(const std::vector<RegexChar> &sv);

std::vector<uint8_t> escapeSingleCRegexChar(RegexChar c);
std::vector<uint8_t>
escapeRegexCLiteralCharBuffer(const std::vector<RegexChar> &sv);

// In the parser if we find an invalid literal character or code then generate
// the right way to escape it for a nice error msg
...
// If we have decode failures then go through and generate nice messages for
// them
std::vector<std::u8string>
parserValidateEscapeSequences(bool ischar, const uint8_t *s, const uint8_t *e);

// Scan the string and ensure that there are no multibyte chars that have messed
// up encodings -- or that the string is only char chars
std::optional<std::u8string> parserValidateUTF8ByteEncoding(const uint8_t *s,
                                                            const uint8_t *e);
std::optional<std::u8string> parserValidateAllCEncoding(const uint8_t *s,
                                                        const uint8_t *e);

std::optional<std::u8string>
parserValidateUTF8ByteEncoding_SingleChar(const uint8_t *s,
                                          const uint8_t *epos);
std::optional<std::u8string>
parserValidateAllCEncoding_SingleChar(const uint8_t *s, const uint8_t *epos);
} // namespace brex
